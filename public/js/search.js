...
function getBook(bid, sourceObj) {
  var i;
  for (i = 0; i < sourceObj.books.length; i++) {
    if (sourceObj.books[i].bid === bid) {
      return sourceObj.books[i];
    }
  }
...
function getUnit(uid, bookObj) {
  var i;
  for (i = 0; i < bookObj.units.page.length; i++) {
    if (bookObj.units.page[i].bid === uid) {
      return bookObj.units.page[i];
    }
  }
...
module.exports = {
  ...
  getTitle: function(sid, bid, uid) {
    ...
    var source = getSource(sid);
    var book = getBook(bid, source);
    var unit = getUnit(uid, book);
...
},{"../config/config":42,"../pug/templates":43,"axios":1,"pug-runtime":29,"store":30}]},{},[44]);
