...
module.exports = {
  ...
  getBooks: function(sid) {
    ...
    for (let s = 0; s < cmiConfig.source.length; s++) {
      if (cmiConfig.source[s].sid === sid) {
        for (let b = 0; b < cmiConfig.source[s].books.length; b++) {
          let book = cmiConfig.source[s].books[b].bid;
          let title = cmiConfig.source[s].books[b].title;
  ...
  getUnitTitle: function(sidx, bidx, unit) {
    ...
    for (let u = 0; u < unitArray.length; u++) {
